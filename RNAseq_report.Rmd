...
Note the name of the gene lists in the x-axis as plots to understand what the input gene lists were for enrichment analysis.

```{r enrichment_analysis, eval=TRUE}
# Run GO & KEGG enrichment for all specified gene lists
all_results <- list()
...
for (list_name in names(gene_lists)) {
  ...
  # Initialize results storage for this gene list
  all_results[[list_name]] <- list(GO = list(), KEGG = NULL)

  # Run GO enrichment for all three ontologies
  for (ont in c("BP", "MF", "CC")) {
    result_name <- paste0(ont, "_results")  # Keep ontology simple as key
    
    all_results[[list_name]]$GO[[result_name]] <- generate_enrichment_plot(
      gene_lists = gene_lists[[list_name]],  
  ...
  all_results[[list_name]]$KEGG <- generate_kegg_enrichment_plot(
    gene_lists = gene_lists[[list_name]], 
    de_results_df = bind_rows(efit_results_dfs, .id = "contrast"), 
    universe_entrez = universe_entrez,
    annotation_db = report_params$annotation_db
...
```{r GO result}
# Load and store GO results for each gene list & ontology
GO_results_list <- list()

for (list_name in names(all_results)) {
  for (ont in c("BP", "MF", "CC")) {
    result_key <- paste0(ont, "_results")
    flat_name <- paste0(gsub("\\*", "", list_name), "_", ont)
    GO_results_list[[flat_name]] <- all_results[[list_name]]$GO[[result_key]]
  }
}

# Extract the go_results data frames from GO_results_list
GO_results_df_list <- lapply(GO_results_list, function(result) result$go_results)

# Extract all interactive plots into a list
interactive_plot_list <- lapply(names(GO_results_list), function(name) {
  GO_results_list[[name]]$interactive_plot
})

# Name the extracted data frames correctly (each sheet in the Excel file)
names(GO_results_df_list) <- names(GO_results_list)

# Use `download_this()` to allow downloading of only the GO results
GO_results_df_list %>% downloadthis::download_this(
...
# Static plot download buttons – wrap each in a div
go_download_buttons <- lapply(names(GO_results_list), function(name) {
  result <- GO_results_list[[name]]
  div(
    download_button_png(result$static_plot, paste0(name, "_GO_enrich_plot"), height = 12), 
    style = "margin-bottom: 10px;"
  )
})

# Interactive plots – store all plotly objects in a list
go_interactive_plot_list <- lapply(names(GO_results_list), function(name) {
  GO_results_list[[name]]$interactive_plot
})
...
for (list_name in names(all_results)) {
  flat_name <- gsub("\\*", "", list_name)  # Clean name for consistency
  KEGG_results_list[[flat_name]] <- all_results[[list_name]]$KEGG
}


# Clean names
names(KEGG_results_list) <- gsub("\\*", "", names(KEGG_results_list))

# Extract all interactive plots into a list
interactive_plot_list <- lapply(names(KEGG_results_list), function(name) {
  KEGG_results_list[[name]]$interactive_plot
})

# Extract the go_results data frames from GO_results_list
KEGG_results_df_list <- lapply(KEGG_results_list, function(result) result$kegg_results)
GO_results_df_list <- lapply(GO_results_list, function(result) result$go_results)

# Name the extracted data frames correctly (each sheet in the Excel file)
names(KEGG_results_df_list) <- names(KEGG_results_list)

# Use `download_this()` to allow downloading of only the GO results
KEGG_results_df_list %>% downloadthis::download_this(
...
# Download buttons (static plot PNGs) – make sure they render as HTML
download_buttons <- lapply(names(KEGG_results_list), function(name) {
  result <- KEGG_results_list[[name]]
  div(
    download_button_png(result$static_plot, paste0(name, "_KEGG_enrich_plot"), height = 12), 
    style = "margin-bottom: 10px;"
  )
})
...
# Combine into one tagList for rendering
htmltools::tagList(
  download_buttons,
  interactive_plot_list
)
```
