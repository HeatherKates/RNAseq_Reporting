...
# Load the parameters
args <-  params$args
report_params <- parse_params(args[1])
```
...
# Loop through all gene lists
for (list_name in names(all_results)) {
  
  cat("\n\n## Gene Ontology (GO) Enrichment: ", list_name, "\n\n")
  
  # Loop through GO categories (BP, MF, CC)
  for (ont in c("BP", "MF", "CC")) {
    
    # Extract results
    go_result <- all_results[[list_name]]$GO[[paste0(ont, "_results")]]
    
    if (!is.null(go_result)) {
      
      # Print section header dynamically
      cat("\n\n### GO ", ont, " Enrichment \n\n")
      
      # store for later use in `lapply()`
      GO_results_list[[paste0(list_name, "_", ont)]] <- go_result
    }
  }
}
# clean names
names(GO_results_list) <- gsub("\\*","",names(GO_results_list))

# Download GO results as an excel file
GO_results_df_list <- lapply(GO_results_list, function(result) result$go_results)

# Name the extracted data frames correctly
...
GO_results_df_list %>% downloadthis::download_this(
  ...
  output_extension = ".xlsx",
  button_label = "Download GO results",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save"
)
...
for (list_name in names(all_results)) {
  ...
  cat("\n\n## KEGG Pathway Enrichment: ", list_name, "\n\n")
  
  # Extract KEGG results
  kegg_result <- all_results[[list_name]]$KEGG
  
  if (!is.null(kegg_result)) {
    
    # Store for later use
    KEGG_results_list[[list_name]] <- kegg_result
  }
}
...
# Download KEGG results as an Excel file
...
# Use `download_this()` to allow downloading of only the KEGG results
KEGG_results_df_list %>% downloadthis::download_this(
  output_name = "KEGG_results",
  output_extension = ".xlsx",
  button_label = "Download KEGG results",
  button_type = "primary",
  has_icon = TRUE,
  icon = "fa fa-save"
)
...
# Static plot downloads
lapply(names(KEGG_results_list), function(name) {
  result <- KEGG_results_list[[name]]
  download_button_png(result$static_plot, paste0(name, "_KEGG_enrich_plot"), height = 12)
})

# Store all interactive plots in a list
kegg_interactive_plot_list <- lapply(names(KEGG_results_list), function(name) {
  KEGG_results_list[[name]]$interactive_plot
})

# Render all plots properly
tagList(kegg_interactive_plot_list)
```
