...
# Load the parameters
report_params <- parse_params("Report_params.txt")
```

```{r}
suppressMessages(suppressWarnings(capture.output(source("analysis.R"), file = NULL)))
```
...
for (i in seq_along(efit_list)) {
  contrast=names(efit_list)[i]
  print(display_de_summary(efit_list[[i]]))  # Explicit print
}
```
...
for (i in seq_along(efit_list)) {
  ...
  print(display_de_result_table(efit_list[[i]], contrast_name = contrast_name))  # Explicit print
}
```
...
```{r heatmaps of top DE genes,fig.width=9, fig.height=5}
