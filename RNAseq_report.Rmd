...
for (list_name in names(gene_lists)) {
  ...
  for (ont in c("BP", "MF", "CC")) {
    result_name <- paste0(list_name, "_GO_", ont, "_results")
    
    GO_results[[result_name]] <- generate_enrichment_plot(
    ...
    saveRDS(GO_results[[result_name]], paste0(report_params$out_dir, "/", result_name, ".RDS"))
  }
  
  # Perform KEGG enrichment
  KEGG_result_name <- paste0(list_name, "_KEGG_results")
  ...
  saveRDS(KEGG_results, paste0(report_params$out_dir, "/", KEGG_result_name, ".RDS"))
  
  # Store results for rendering
  all_results[[list_name]] <- list(GO = GO_results, KEGG = KEGG_results)
}
```
...
```{r GO result}
# Load and render GO results for each gene list & ontology
for (list_name in names(all_results)) {
  for (ont in c("BP", "MF", "CC")) {
    ...
    if (!exists(result_name)) {
      all_results[[list_name]]$GO[[result_name]] <- readRDS(paste0(report_params$out_dir, "/", result_name, ".RDS"))
    }
    
    # Extract results
    go_result <- all_results[[list_name]]$GO[[result_name]]
    
    # Plot interactive and static
    download_button_png(go_result$static_plot, paste0(result_name, "_plot"), height=12)
    print(go_result$interactive_plot)
    
    # Downloadable Excel file
    go_result$go_results %>% downloadthis::download_this(
      output_name = result_name,
      output_extension = ".xlsx",
      button_label = paste("Download", list_name, ont, "GO results"),
      button_type = "primary",
      has_icon = TRUE,
      icon = "fa fa-save"
    )
...
```{r}
# Load and render KEGG results for each gene list
for (list_name in names(all_results)) {
  KEGG_result_name <- paste0(list_name, "_KEGG_results")
  
  # Load saved KEGG results if needed
  if (!exists(KEGG_result_name)) {
    all_results[[list_name]]$KEGG <- readRDS(paste0(report_params$out_dir, "/", KEGG_result_name, ".RDS"))
  }
  ...
  # Plot interactive and static
  download_button_png(kegg_result$static_plot, paste0(KEGG_result_name, "_plot"), height=12)
  print(kegg_result$interactive_plot)
  
  # Downloadable Excel file
  kegg_result$kegg_results %>% downloadthis::download_this(
    output_name = KEGG_result_name,
    output_extension = ".xlsx",
    button_label = paste("Download", list_name, "KEGG results"),
    button_type = "primary",
    has_icon = TRUE,
    icon = "fa fa-save"
  )
