...
download_button <- GO_results_df_list %>% downloadthis::download_this(
...
# Render the download button separately
div(download_button, style = "margin-bottom: 20px;")

# Render static plot download buttons, ensuring each is on its own line
unlist(lapply(names(GO_results_list), function(name) {
  result <- GO_results_list[[name]]
  div(download_button_png(result$static_plot, paste0(name, "_GO_enrich_plot"), height = 12), 
...
# Use `download_this()` to allow downloading of only the GO results
download_button <- KEGG_results_df_list %>% downloadthis::download_this(
...
# Render static plot download buttons, ensuring each is on its own line
unlist(lapply(names(KEGG_results_list), function(name) {
  result <- KEGG_results_list[[name]]
  div(download_button_png(result$static_plot, paste0(name, "_KEGG_enrich_plot"), height = 12), 
      style = "margin-bottom: 10px;")
}), recursive = FALSE)
